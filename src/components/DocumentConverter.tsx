...
`.trim();
  ...
  const processChunksSequentially = async (
        ...
        while (
          ...
          try {
            ...
            if (err?.handled) {
              ...
              pushLog(
                `⚠️ [Khối ${i + 1}] Lỗi đã được xử lý bằng popup: ${lastErrorMessage}`,
                true,
              );
              success = false;
            ...
            if (
              ...
              pushLog(
                `❄️ Lùi lại ${currentBackoff / 1000} giây trước khi tự động truy vấn lại...`,
              );
    ...
    const actualConcurrency = Math.min(
      concurrency,
      totalCount - sessionData.activeProgressIdx,
    );
      ...
      const worker = async (workerId: number) => {
        while (currentIndex < cardsToHydrate.length) {
          ...
          try {
            ...
            console.warn(
              `[Hydration Worker #${workerId}] ⚠️ Không thể tự nạp cho từ '${card.front}':`,
              err.message || err,
            );
          }
