...
export default function TeacherDashboard() {
  ...
  const handleDispatchReward = async () => {
    ...
    try {
      ...
      setRewardMessage(`Thành công! Đã ban tặng cho ${chosenUser.name}: ${addPoints >= 0 ? '+' : ''}${addPoints} PT | ${addLevel >= 0 ? '+' : ''}${addLevel} Level | ${addStreak >= 0 ? '+' : ''}${addStreak} Streak | ${addStudyMinutes >= 0 ? '+' : ''}${addStudyMinutes} Mins | ${addTop1Weeks >= 0 ? '+' : ''}${addTop1Weeks} Top 1.`);
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <div className="space-y-8">
          ...
          <section className="glass p-6 rounded-2xl relative overflow-hidden border border-orange-500/20 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
              ...
              <div>
                <label className="block text-xs font-bold mb-1 opacity-80">Top 1 Tuần</label>
                <input
                  type="text"
                  placeholder="Ví dụ: +1 | -1"
        ...
        <div className="space-y-8">
          <section className="glass p-6 rounded-2xl flex flex-col">
            <h3 className="text-xl font-display font-bold flex items-center gap-2 mb-2"><AlertCircle className="w-5 h-5 text-red-500" /> AI Weakness Detection</h3>
            <p className="text-sm opacity-70 mb-2">Thống kê các khái niệm lớp học đang yếu (thẻ được đánh dấu "Khó" hoặc điểm SM-2 cực do hệ thống gốc thiết lập).</p>
            <p className="text-xs bg-blue-500/10 text-blue-700 dark:text-blue-300 p-2 rounded-lg mb-6 border border-blue-500/20">
              <span className="font-bold">Lưu ý về cơ chế đánh dấu X:</span> Khi học sinh nhấn nút "X" (Quên) trong lúc ôn tập, kết quả đó được lưu vào tiến trình bộ nhớ <b>cá nhân</b> của học sinh đó và không hiển thị tự động lên bảng thống kê gốc này (để tránh làm nghẽn dữ liệu bộ bài dùng chung). Bảng này chỉ đo lường điểm cấu hình gốc từ Bộ Thẻ của lớp.
            </p>
